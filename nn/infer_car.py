## python -m nn.infer_Car from the DEB directory
...
def run():
    ...
    data = pass # You are getting data from carla. It is in the same format as the pkl files
    ...
    t1 = time.time()
    sampler1.infer_traj()
    mean_controls = sampler1.mean_action # contains the mean v and w
    mean_traj = sampler1.traj_N[-2,:,:] # contains the mean x, y and theta
    best_controls = sampler1.top_controls[0,:,:] # contains the best v and w
    best_traj = sampler1.top_trajs[0,:,:] # contains the best x, y and theta
    # USE 0-th index on the car and query the NN again 
