...
def infer_dataset():
    ...
    weights = torch.load(args.weights_dir+"model_"+args.exp_id+".pt",
    ...
    for datapoint in dataset:
        ...
        sampler1 = Goal_Sampler(torch.tensor([0, 0, np.deg2rad(90)]), 4.13, 0,
                                obstacles=obs_pos, num_particles=100)
        sampler1.mean_action = output.reshape(30, 2).detach().cpu()
        sampler1.infer_traj()

        sampler2 = Goal_Sampler(torch.tensor([0, 0, np.deg2rad(90)]), 4.13, 0,
                                obstacles=obs_pos, num_particles=100)
        ...
        plt.plot(sampler1.traj_N[:, :, 0], sampler1.traj_N[:, :, 1],
                 '.b', markersize=1, alpha=0.04)
        plt.plot(sampler2.traj_N[-2, :, 0], sampler2.traj_N[-2, :, 1],
                 'orange', markersize=3,  label="Ground Truth")
        plt.plot(sampler1.traj_N[-2, :, 0], sampler1.traj_N[-2, :, 1],
                 'red', markersize=3, label="Predicted")
        plt.plot(sampler1.top_trajs[0, :, 0], sampler1.top_trajs[0, :, 1],
                 'lime', markersize=2, label="NN best traj")
        plt.plot(sampler2.top_trajs[0, :, 0], sampler2.top_trajs[0, :, 1],
                 'pink', markersize=2, label="GT best traj")
