...
@dataclass
class Args:
    occ_map_dir: str = '/Users/kaustabpal/Downloads/new_data/occ_map/'
    mean_dir: str = '/Users/kaustabpal/Downloads/new_data/mean_controls/'
    plot_im_dir: str = '/Users/kaustabpal/Downloads/new_data/plot_im/'
    # val_split: float = 0.3
    # num_epochs: int = 500
    # seed: int = 12321
    exp_id: str = 'exp2'
...
def run():
    ...
    for i in range(0, len(files)):
        ...
        obs_pos_frenet = global_to_frenet(obs_pos, new_g_path,
                                          interpolated_g_path)

        sampler = Goal_Sampler(torch.tensor([0, 0, np.deg2rad(ego_theta)]), 4,
                               0, obstacles=obs_pos_frenet)
        t1 = time.time()
        sampler.plan_traj()
        ...
        mean_controls[:, 1] = frenet_to_global(mean_traj, new_g_path,
                                               interpolated_g_path, 0.1)
        # print(mean_controls)
        # quit()
        ...
        plt.plot(sampler.traj_N[:, :, 0], sampler.traj_N[:, :, 1],
                 '.r', alpha=0.01)
